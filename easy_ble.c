...
static const char *GATTS_TAG = "easy_ble";

static easy_ble_callback_t rx_callback = 0;
...
#define TEST_DEVICE_NAME            "EASY BLE OTA"
...
static uint8_t adv_service_uuid128[16] = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x53, 0x4c, 0x9e, 0xbc, 0x29, 0x28, 0x07, 0x55
};
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        memcpy(gl_profile_tab[PROFILE_A_APP_ID].service_id.id.uuid.uuid.uuid128, adv_service_uuid128, sizeof(adv_service_uuid128)) ;
    ...
    case ESP_GATTS_WRITE_EVT: {
        if (!param->write.is_prep){ 
            if(iot47_ble_ota_task(param->write.value,param->write.len,&gatts_if,param->write.conn_id,param->write.handle) == 0)
            {
                ...
                if(rx_callback != 0)rx_callback(param->write.value,param->write.len,&gatts_if,param->write.conn_id,param->write.handle);
            }
        }
...
{
    for(int i=0;i<6;i++)adv_service_uuid128[i] = mac_addr[i];
